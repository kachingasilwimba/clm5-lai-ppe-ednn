...
# # =============================================================================
# # data_processing/annual_data_loader.py
# # =============================================================================
# import numpy as np
# import pandas as pd
# import xarray as xr
...
# PPE_CSV   = cfg["file_paths"]["ppe_csv"]
# CHUNKS    = cfg["chunk_sizes"]

# def x_input_xr(param_file_path: str=None, times: np.ndarray=None):
#     """
#     Build an xarray.Dataset of PPE parameters + cyclical month (sin/cos)
#     and year features. Does not materialize a large 2D table.
#     Returns (X_features, params_raw).
#     """
#     #---------- 1) Read in PPE CSV, convert to xarray.Dataset
#     path    = param_file_path or PPE_CSV
#     ppe_df  = pd.read_csv(path)
#     params  = xr.Dataset.from_dataframe(ppe_df.set_index("member"))
#     members = ppe_df["member"].values

#     #---------- 2) Build time-based features
#     years  = np.array([t.year for t in times],  dtype="int16")
#     months = np.array([t.month for t in times])
#     m_sin  = np.sin(2*np.pi * months/12.0).astype("float32")
#     m_cos  = np.cos(2*np.pi * months/12.0).astype("float32")
...
#     #---------- 3) Assemble feature Dataset
#     X = xr.Dataset()
#     # 3a) Broadcast PPE params across time
#     for name in params.data_vars:
#         X[name] = params[name] \
#             .expand_dims(time=times) \
#             .transpose("member", "time")

#     #---------- 3b) Broadcast cyclical/year features across members
#     for feat, arr in [("m_sin", sin_da), ("m_cos", cos_da), ("year", yr_da)]:
#         X[feat] = arr \
#             .expand_dims(member=members) \
#             .transpose("member", "time")

#     #---------- 4) Chunk the Dataset for Dask
#     X = X.chunk({"member": CHUNKS["member"], "time": CHUNKS["time"]})
